...
class LayerParent {
    ...
    void setWeights(vector<vector<double>> updated_weights)
    {
        weights = updated_weights;
    }
    void setBiases(vector<vector<double>> updated_biases)
    {
...
public:
    ...
    NeuralNetwork(vector<int> neuronCount, double alpha, string fileName = "na")
    {
        ...
        if (fileName != "na")
        {
            ...
            if (!inFile.is_open())
            {
                std::cerr << "Error: Model File could not be opened or found!" << std::endl;
            }
            for (int i = 0; i < this->layers.size(); i++)
            {
                this->loadLayer(inFile, layers[i]->weights, layers[i]->biases);
                cout << " " << i << " W: " << layers[i]->weights[0][0] << " B: " << layers[i]->biases[0][0] << endl;
            }
            inFile.close();
        }
    }
    ...
    {
        std::ios_base::openmode mode = std::ios::app;
        std::ofstream outFile("C:\\Users\\Darshan\\Desktop\\OOP project\\model\\metrics\\accuracy.txt", mode);
        if (!outFile)
        {
            std::cerr << "Error opening file for writing Loss!" << std::endl;
            return;
