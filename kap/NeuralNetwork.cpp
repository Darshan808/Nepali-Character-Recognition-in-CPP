...
int LayerParent::getNumberOfNeurons()
{
    return biases.size();
}
vector<vector<double>> LayerParent::getWeights()
{
    return weights;
}
void LayerParent::setWeights(vector<vector<double>> updated_weights)
{
    weights = updated_weights;
}
void LayerParent::setBiases(vector<vector<double>> updated_biases)
{
    biases = updated_biases;
}
vector<vector<double>> LayerParent::getBiases()
{
    return biases;
}
...
void NeuralNetwork::leakyRelu(std::vector<std::vector<double>> &mat)
{
    for (auto &row : mat)
    {
        for (auto &val : row)
...
void NeuralNetwork::softmax(vector<vector<double>> &matOrg)
{
...
NeuralNetwork::NeuralNetwork(vector<int> neuronCount, double alpha, string fileName = "na")
{
    this->alpha = alpha;
...
<vector<double>> NeuralNetwork::feedForward(vector<vector<double>> input)
{
...
vector<vector<double>> NeuralNetwork::deriv_LeakyReLU(const vector<vector<double>> &mat)
{
...
void NeuralNetwork::backward_prop(vector<vector<double>> X, vector<int> Y)
{
    int m = Y.size();
...
vector<int> NeuralNetwork::(const vector<vector<double>> &P)
{
...
double NeuralNetwork::(vector<int> &Y, vector<vector<double>> &predictions)
{
    double loss = 0;
...
void NeuralNetwork::saveAccuracies()
{
    std::ios_base::openmode mode = std::ios::app;
...
void NeurralNetwork::saveLayer(vector<vector<double>> &weights, vector<vector<double>> &biases, std::string &filename, bool reset = false)
{
...
bool NeuralNetwork::loadLayer(ifstream &inFile, vector<vector<double>> &W, vector<vector<double>> &B)
{
    string line;
    int row, col;
    char type;
...
void NeuralNetwork::train(vector<vector<double>> X_train, vector<int> Y_train, vector<vector<double>> X_val, vector<int> Y_val, vector<vector<double>> X_val_2, vector<int> Y_val_2, int iterations, bool verbose = false, string fileName = "model.txt", bool save = false)
{
